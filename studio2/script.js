(function () {
	...
	window.addEventListener('load', function () {
		let figImage = document.querySelector('figure img');
		...
		window.addEventListener('scroll', function () {
			...
			// prevent zoom on scroll for first image 
			if (counter === 2)
			{
				figImage.className = 'sect1';
			}
			// start zoom on scroll after first image
			else if (counter != prevCounter) {
				
				figImage.className = 'sect' + counter;
